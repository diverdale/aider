...
class TestMCPChunkLoop(TestCase):
    ...
    def test_single_tool_call_accumulates_id_name_and_args(self):
        ...
        self._consume_stream(
            [
                _delta_chunk(
                    tool_calls=[
                        _tool_call_delta(0, id="call_1", name="mcp__fs__read", arguments='{"path"'),
                    ]
                ),
                _delta_chunk(
                    tool_calls=[
    ...
    def test_text_and_tool_calls_coexist(self):
        ...
        self._consume_stream(
            [
                ...
                _delta_chunk(
                    tool_calls=[
                        _tool_call_delta(
                            0, id="c0", name="mcp__fs__read", arguments='{"path": "/x"}'
                        ),
                    ]
                ),
        ...
        self.assertEqual(self.coder.partial_response_content, "I'll read the file. Done.")
        self.assertEqual(len(self.coder.partial_tool_calls), 1)
        self.assertEqual(self.coder.partial_tool_calls[0]["name"], "mcp__fs__read")
    ...
    def test_no_tool_calls_leaves_state_empty(self):
        ...
        self.assertEqual(self.coder.partial_response_content, "Just text, no tools.")
